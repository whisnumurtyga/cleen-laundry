...
const nearbyPlaces = (latitude, longitude, type = "laundry") =>
  axios.get(
    `${BASE_URL}/nearbysearch/json?location=${latitude},${longitude}&radius=500&type=${type}&key=${API_KEY}`
  );
...
const getPlacePhotoById = (width = 400, photo_reference = "Surabaya") =>
  axios.get(
    `${BASE_URL}/photo?maxwidth=${width}&photo_reference=${photo_reference}&key=${API_KEY}`
  );
